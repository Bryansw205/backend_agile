...
// Calcula cronograma de pagos con cuotas fijas mensuales.
// interestRate es anual (ej. 0.24 para 24%).
// termCount: número de meses.
export function generateSchedule({ principal, interestRate, termCount, startDate }) {
  ...
  for (let i = 1; i <= periods; i++) {
    ...
    let dueDate = start.add(i, 'month');
    // Fijar mediodía para evitar desfaces por TZ/DST al serializar
    dueDate = dueDate.hour(12).minute(0).second(0);
  ...
  if (last) {
    ...
    // 3) Alinear la última cuota = capital + interés (puede variar ±0.01 respecto a la cuota fija)
